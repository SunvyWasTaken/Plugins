...
void USunLibraryBPLibrary::F_AddQuality(UComboBoxString* target)
{
	FString option;
	for (int i = 0; i < 4; i++)
	{
...
void USunLibraryBPLibrary::F_AddShadowResolution(UComboBoxString* target)
{
	F_AddQuality(target);
	target->SetSelectedIndex(UGameUserSettings::GetGameUserSettings()->GetShadowQuality());
	return;
}
